...
impl App {
    ...
    fn open_agent_role_picker(&mut self) {
        if self.thread_event_channels.is_empty() {
            self.chat_widget
                .add_info_message("No agents available yet.".to_string(), None);
            return;
        ...
        for role_id in &self.config.plugins.ui().tab_picker_roles {
            let Some(role) = self.config.plugins.role(role_id) else {
                continue;
            };
            let is_current = role.id == current_role;
            if is_current {
                initial_selected_idx = Some(items.len());
            }

            let role_name = role.id.clone();
            items.push(SelectionItem {
                name: role.label.clone(),
                description: role.description.clone(),
                is_current,
                actions: vec![Box::new(move |tx| {
                    tx.send(AppEvent::SelectAgentRole(role_name.clone()));
                })],
                dismiss_on_select: true,
                search_value: Some(role.id.clone()),
                ..Default::default()
            });
        }
        ...
        if items.is_empty() {
            self.chat_widget.add_info_message(
                "No agent roles available.".to_string(),
                Some("The active plugin did not define tab_picker_roles.".to_string()),
            );
            return;
        }
        ...
        self.chat_widget.show_selection_view(SelectionViewParams {
            title: Some("Agents".to_string()),
            subtitle: Some("Select an agent role/persona".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            initial_selected_idx,
            is_searchable: true,
            search_placeholder: Some("Search roles…".to_string()),
            ..Default::default()
        });
    }

    async fn select_agent_role(&mut self, tui: &mut tui::Tui, role: String) -> Result<()> {
        let role = normalize_role_id(&role);
        if role.is_empty() {
            return Ok(());
        }

        let (role_id, role_model, role_reasoning_effort) = match self.config.plugins.role(&role) {
            Some(role_def) => (
                role_def.id.clone(),
                role_def.model.clone(),
                role_def.reasoning_effort,
            ),
            None => {
                self.chat_widget
                    .add_error_message(format!("Unknown agent role: {role}"));
                return Ok(());
            }
        };
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::UpdateAgentSpawnAllow { allow } => {
                self.config.agent_spawn.allow = allow.clone();
                self.chat_widget.set_agent_spawn_allow(allow.clone());

                let profile = self.active_profile.as_deref();
                match ConfigEditsBuilder::new(&self.config.codex_home)
                    .with_profile(profile)
                    .set_agent_spawn_allow(&allow)
                    .apply()
                    .await
                {
                    Ok(()) => {
                        let mut message = "Saved agent spawn permissions".to_string();
                        if let Some(profile) = profile {
                            message.push_str(" for ");
                            message.push_str(profile);
                            message.push_str(" profile");
                    ...
                    Err(err) => {
                        tracing::error!(error = %err, "failed to persist agent spawn allowlist");
                        if let Some(profile) = profile {
                            self.chat_widget.add_error_message(format!(
                                "Failed to save agent spawn permissions for profile `{profile}`: {err}"
                            ));
                        } else {
                            self.chat_widget.add_error_message(format!(
                                "Failed to save agent spawn permissions: {err}"
                            ));
                        }
                    }
                }
            }
            AppEvent::UpdateAgentRoleModelSeries { series } => {
                self.config.agent_role_models.series = series.clone();
                self.chat_widget.set_agent_role_model_series(series.clone());

                let profile = self.active_profile.as_deref();
                match ConfigEditsBuilder::new(&self.config.codex_home)
                    .with_profile(profile)
                    .set_agent_role_models_series(&series)
                    .apply()
                    .await
                {
                    Ok(()) => {
                        let mut entries = series
                            .iter()
                            .map(|(role_series, model)| {
                                format!("{} -> {model}", role_series.as_str())
                            })
                            .collect::<Vec<_>>();
                        entries.sort();
                        let mut message = if entries.is_empty() {
                            "Cleared role-series model overrides".to_string()
                        } else {
                            format!("Saved role-series model overrides: {}", entries.join(", "))
                        };
                        if let Some(profile) = profile {
                            message.push_str(" for ");
                            message.push_str(profile);
                    ...
                    Err(err) => {
                        tracing::error!(error = %err, "failed to persist role-series model overrides");
                        if let Some(profile) = profile {
                            self.chat_widget.add_error_message(format!(
                                "Failed to save role-series model overrides for profile `{profile}`: {err}"
                            ));
                        } else {
                            self.chat_widget.add_error_message(format!(
                                "Failed to save role-series model overrides: {err}"
                            ));
                        }
                    }
                }
            ...
            AppEvent::SelectAgentRole(role) => {
                self.select_agent_role(tui, role).await?;
            }
            AppEvent::OpenRoleModelSeriesPrompt { series } => {
                self.chat_widget.show_role_model_series_prompt(series);
            }
