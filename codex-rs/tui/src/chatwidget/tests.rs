...
#[tokio::test]
async fn plan_implementation_popup_yes_in_plan_mode_emits_default_mode_event() {
...
#[tokio::test]
async fn plan_implementation_popup_yes_in_heavy_plan_mode_emits_heavy_mode_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    let heavy_plan_mask =
        collaboration_modes::mask_for_kind(chat.models_manager.as_ref(), ModeKind::HeavyPlan)
            .expect("expected heavy plan collaboration mask");
    chat.set_collaboration_mask(heavy_plan_mask);
    ...
    assert_eq!(text, PLAN_IMPLEMENTATION_CODING_MESSAGE);
    assert_eq!(collaboration_mode.mode, Some(ModeKind::Heavy));
}
...
#[tokio::test]
async fn heavy_plan_implementation_popup_shows_after_proposed_plan_output() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    let heavy_plan_mask =
        collaboration_modes::mask_for_kind(chat.models_manager.as_ref(), ModeKind::HeavyPlan)
...
async fn collab_mode_shift_tab_cycles_only_when_enabled_and_idle() {
    ...
    chat.handle_key_event(KeyEvent::from(KeyCode::BackTab));
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Heavy);
    assert_eq!(chat.current_collaboration_mode(), &initial);

    chat.handle_key_event(KeyEvent::from(KeyCode::BackTab));
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::HeavyPlan);
    assert_eq!(chat.current_collaboration_mode(), &initial);
...
#[tokio::test]
async fn role_models_show_reports_current_overrides() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.config.agent_role_models.series = HashMap::from([
        (AgentRoleModelSeries::Heavy, "gpt-5.3-codex".to_string()),
        (AgentRoleModelSeries::Normal, "gpt-5.2".to_string()),
    ]);

    chat.bottom_pane
        .set_composer_text("/role-models show".to_string(), Vec::new(), Vec::new());
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
    ...
    assert!(text.contains("Role-series model overrides:"));
    assert!(text.contains("heavy -> gpt-5.3-codex"));
    assert!(text.contains("normal -> gpt-5.2"));
}

#[tokio::test]
async fn role_models_command_without_args_opens_interactive_popup() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.config.agent_role_models.series = HashMap::from([
        (AgentRoleModelSeries::Fast, "gpt-5.3-codex".to_string()),
        (AgentRoleModelSeries::Heavy, "gpt-5.2".to_string()),
    ]);

    chat.dispatch_command(SlashCommand::RoleModels);

    let popup = render_bottom_popup(&chat, 100);
    assert!(popup.contains("Role-series model overrides"));
    assert!(popup.contains("Set fast model"));
    assert!(popup.contains("Set normal model"));
    assert!(popup.contains("Set heavy model"));
    assert!(popup.contains("Current override: gpt-5.3-codex"));
    assert!(popup.contains("Current override: inherit role default"));
}

#[tokio::test]
async fn role_models_popup_set_option_opens_series_prompt() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    let mut found = false;
    while let Ok(event) = rx.try_recv() {
        if let AppEvent::OpenRoleModelSeriesPrompt { series } = event {
            assert_eq!(series, AgentRoleModelSeries::Fast);
            found = true;
            break;
        }
    }
    assert!(found, "expected OpenRoleModelSeriesPrompt event");
}

#[tokio::test]
async fn role_models_series_prompt_submission_emits_update_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.show_role_model_series_prompt(AgentRoleModelSeries::Heavy);
    chat.handle_paste("gpt-5.3-codex".to_string());
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));

    let mut found = false;
    while let Ok(event) = rx.try_recv() {
        if let AppEvent::UpdateAgentRoleModelSeries { series } = event {
            assert_eq!(
                series.get(&AgentRoleModelSeries::Heavy),
    ...
    assert!(found, "expected UpdateAgentRoleModelSeries event");
}

#[tokio::test]
async fn role_models_set_emits_update_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.bottom_pane.set_composer_text(
        "/role-models set heavy gpt-5.3-codex".to_string(),
        Vec::new(),
        Vec::new(),
    );
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
...
#[tokio::test]
async fn role_models_unknown_action_reports_error() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.bottom_pane
        .set_composer_text("/role-models boom".to_string(), Vec::new(), Vec::new());
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
    ...
    let lines = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>();
    let text = lines_to_single_string(&lines);
    assert!(text.contains("Unknown /role-models action `boom`"));
}
...
#[tokio::test]
async fn slash_context_requests_prepared_context() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command(SlashCommand::Context);

    match op_rx.try_recv() {
        Ok(Op::GetPreparedContextRequest) => {}
        Ok(other) => panic!("expected Op::GetPreparedContextRequest, got {other:?}"),
        Err(err) => panic!("expected context op, got {err:?}"),
    }
}

#[tokio::test]
async fn prepared_context_response_displays_snapshot_json() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.handle_codex_event(Event {
        id: "ctx-1".to_string(),
        msg: EventMsg::PreparedContextResponse(PreparedContextResponseEvent {
            snapshot_json: Some("{\n  \"input\": []\n}".to_string()),
        }),
    });

    let cells = drain_insert_history(&mut rx);
    assert_eq!(cells.len(), 1, "expected one context output cell");
    let rendered = lines_to_single_string(&cells[0]);
    assert!(
        rendered.contains("/context"),
        "expected context header in output: {rendered}"
    );
    assert!(
        rendered.contains("\"input\": []"),
        "expected snapshot payload in output: {rendered}"
    );
}

#[tokio::test]
async fn prepared_context_response_without_snapshot_shows_hint() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    let cells = drain_insert_history(&mut rx);
    assert_eq!(cells.len(), 1, "expected informational fallback cell");
    let rendered = lines_to_single_string(&cells[0]);
    assert!(
        rendered.contains("not available yet"),
        "expected not-available message: {rendered}"
    );
    assert!(
        rendered.contains("try /context again"),
        "expected retry hint in message: {rendered}"
    );
}
