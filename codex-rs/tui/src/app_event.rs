...
pub(crate) enum AppEvent {
    ...
    /// Switch the active agent role/persona (plugin-driven).
    SelectAgentRole(String),
    ...
    /// Update agent spawn permissions and persist them to config.toml.
    UpdateAgentSpawnAllow {
        allow: HashMap<String, Vec<String>>,
    },

    /// Update role-series model overrides and persist them to config.toml.
    UpdateAgentRoleModelSeries {
        series: HashMap<AgentRoleModelSeries, String>,
    },

    /// Open the editor prompt for one role-model series override.
    OpenRoleModelSeriesPrompt {
        series: AgentRoleModelSeries,
    },
