...
pub(crate) fn implementation_mode_mask(
    models_manager: &ModelsManager,
    current_mode: ModeKind,
) -> Option<CollaborationModeMask> {
    match current_mode {
        ModeKind::Plan => default_mode_mask(models_manager),
        ModeKind::HeavyPlan => mask_for_kind(models_manager, ModeKind::Heavy),
        _ => None,
    }
}

pub(crate) const fn is_plan_style_mode(mode: ModeKind) -> bool {
    matches!(mode, ModeKind::Plan | ModeKind::HeavyPlan)
}
