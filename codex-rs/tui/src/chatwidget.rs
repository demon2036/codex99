...
use codex_core::protocol::PreparedContextResponseEvent;
...
use crate::bottom_pane::AgentSpawnConfigView;
...
use codex_core::config::AgentRoleModelSeries;
...
const ROLE_MODELS_USAGE_HINT: &str = "Usage: /role-models show | /role-models set <fast|normal|heavy> <model> | /role-models clear <fast|normal|heavy> | /role-models reset";

const ROLE_MODEL_SERIES_ORDER: [AgentRoleModelSeries; 3] = [
    AgentRoleModelSeries::Fast,
    AgentRoleModelSeries::Normal,
    AgentRoleModelSeries::Heavy,
];

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum RoleModelsAction {
    Show,
    Set,
    Clear,
    Reset,
...
impl ChatWidget {
    ...
    pub(crate) fn open_agent_spawn_config(&mut self) {
        if self.config.plugins.ui().agent_config_callers.is_empty()
            || self.config.plugins.ui().agent_config_targets.is_empty()
        {
            self.add_info_message(
                "Agent spawn config is unavailable.".to_string(),
                Some("Enable a plugin that defines agent_config_callers/targets.".to_string()),
            );
            return;
        }
        ...
        let active_agent = self
            ...
            .agent_role
            .clone()
            .or_else(|| self.config.plugins.default_agent_role().map(str::to_string))
            .unwrap_or_else(|| "default".to_string());
        let view = AgentSpawnConfigView::new(
            config_path,
            active_agent,
            &self.config.agent_spawn.allow,
            &self.config.plugins,
            self.app_event_tx.clone(),
        );
        self.bottom_pane.show_view(Box::new(view));
    ...
    fn on_plan_delta(&mut self, delta: String) {
        if !collaboration_modes::is_plan_style_mode(self.active_mode_kind()) {
            return;
        }
    ...
    fn open_plan_implementation_prompt(&mut self) {
        let implementation_mask = collaboration_modes::implementation_mode_mask(
            self.models_manager.as_ref(),
            self.active_mode_kind(),
        );
        let (implement_actions, implement_disabled_reason) = match implementation_mask {
            Some(mask) => {
            ...
            None => (
                Vec::new(),
                Some("Implementation mode unavailable".to_string()),
            ),
        };
    ...
    pub(crate) fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        match key_event {
            KeyEvent {
                ...
                kind: KeyEventKind::Press,
                ..
            } if !self.config.plugins.ui().tab_picker_roles.is_empty()
                && !self.bottom_pane.is_task_running()
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::AgentConfig => {
                self.open_agent_spawn_config();
            }
            SlashCommand::RoleModels => {
                self.open_role_models_usage();
            }
            ...
            SlashCommand::Context => {
                self.submit_op(Op::GetPreparedContextRequest);
            }
            SlashCommand::Skills => {
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::RoleModels => {
                ...
                self.handle_role_models_command(prepared_args);
                self.bottom_pane.drain_pending_submission_state();
            }
    ...
    fn open_role_models_usage(&mut self) {
        let mut items: Vec<SelectionItem> = Vec::new();

        for role_series in ROLE_MODEL_SERIES_ORDER {
            ...
            let set_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
                tx.send(AppEvent::OpenRoleModelSeriesPrompt {
                    series: role_series,
                });
            })];
            items.push(SelectionItem {
                name: format!("Set {} model", role_series.as_str()),
                description: Some(Self::role_models_override_description(
            ...
            let mut next_series = self.config.agent_role_models.series.clone();
            next_series.remove(&role_series);
            let clear_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
                tx.send(AppEvent::UpdateAgentRoleModelSeries {
                    series: next_series.clone(),
                });
            })];
            items.push(SelectionItem {
                name: format!("Clear {} override", role_series.as_str()),
                description: Some(Self::role_models_override_description(
                    current_override.as_deref(),
                )),
                is_disabled: !has_override,
                disabled_reason: (!has_override)
                    .then(|| format!("No {} override is set", role_series.as_str())),
        ...
        let reset_actions: Vec<SelectionAction> = vec![Box::new(|tx| {
            tx.send(AppEvent::UpdateAgentRoleModelSeries {
                series: HashMap::new(),
            });
        })];
        items.push(SelectionItem {
            name: "Reset all overrides".to_string(),
            description: Some("Remove fast / normal / heavy series overrides.".to_string()),
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            title: Some("Role-series model overrides".to_string()),
            subtitle: Some(
                "Choose a series action (or use inline /role-models commands).".to_string(),
            ),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
    ...
    fn role_models_override_description(current_override: Option<&str>) -> String {
        match current_override {
            Some(model) => format!("Current override: {model}"),
            None => "Current override: inherit role default".to_string(),
        }
    }

    pub(crate) fn show_role_model_series_prompt(&mut self, role_series: AgentRoleModelSeries) {
        let tx = self.app_event_tx.clone();
        let current_override = self
            .config
            .agent_role_models
            ...
            .get(&role_series)
            .cloned();
        let existing_series = self.config.agent_role_models.series.clone();
        let context_label = Some(Self::role_models_override_description(
            current_override.as_deref(),
        ));
        let view = CustomPromptView::new(
            format!("Set {} model", role_series.as_str()),
            "Type model slug and press Enter".to_string(),
            context_label,
            Box::new(move |model_input: String| {
                ...
                let mut next = existing_series.clone();
                next.insert(role_series, model.to_string());
                tx.send(AppEvent::UpdateAgentRoleModelSeries { series: next });
            }),
        );
    ...
    fn handle_role_models_command(&mut self, args: String) {
        let mut parts = args.split_whitespace();
        let Some(action_raw) = parts.next() else {
            self.open_role_models_usage();
            return;
        };

        let action = match action_raw.to_ascii_lowercase().as_str() {
            "show" => Some(RoleModelsAction::Show),
            "set" => Some(RoleModelsAction::Set),
            "clear" => Some(RoleModelsAction::Clear),
            "reset" => Some(RoleModelsAction::Reset),
            _ => None,
        };

        let Some(action) = action else {
            self.add_error_message(format!(
                "Unknown /role-models action `{action_raw}`. {ROLE_MODELS_USAGE_HINT}"
            ));
            return;
        };

        match action {
            RoleModelsAction::Show => {
                let mut headings = vec!["Role-series model overrides:".to_string()];
                let mut pairs = self
                    .config
                    .agent_role_models
                    .series
                    .iter()
                    .map(|(series, model)| format!("{} -> {model}", series.as_str()))
                    .collect::<Vec<_>>();
                pairs.sort();
                if pairs.is_empty() {
                    self.add_info_message(
                        "No role-series model overrides configured.".to_string(),
                        None,
                    );
                } else {
                    headings.push(pairs.join(", "));
                    self.add_info_message(headings.join(" "), None);
                }
            }
            RoleModelsAction::Set => {
                let Some(series_raw) = parts.next() else {
                    self.add_error_message(format!("Missing series. {ROLE_MODELS_USAGE_HINT}"));
                    return;
                };
                let Some(series) = AgentRoleModelSeries::parse(series_raw) else {
                    self.add_error_message(format!(
                        "Unknown series `{series_raw}`. Expected fast | normal | heavy"
                    ));
                    return;
                };
                ...
                if model.is_empty() {
                    self.add_error_message(format!("Missing model slug. {ROLE_MODELS_USAGE_HINT}"));
                    return;
                }
                let mut next = self.config.agent_role_models.series.clone();
                next.insert(series, model.to_string());
                self.app_event_tx
                    .send(AppEvent::UpdateAgentRoleModelSeries { series: next });
            }
            RoleModelsAction::Clear => {
                let Some(series_raw) = parts.next() else {
                    self.add_error_message(format!("Missing series. {ROLE_MODELS_USAGE_HINT}"));
                    return;
                };
            ...
            RoleModelsAction::Reset => {
                self.app_event_tx
                    .send(AppEvent::UpdateAgentRoleModelSeries {
                        series: HashMap::new(),
                    });
            }
        }
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            EventMsg::PreparedContextResponse(ev) => self.on_prepared_context_response(ev),
    ...
    pub(crate) fn set_agent_spawn_allow(&mut self, allow: HashMap<String, Vec<String>>) {
        self.config.agent_spawn.allow = allow;
    }

    pub(crate) fn set_agent_role_model_series(
        &mut self,
        series: HashMap<AgentRoleModelSeries, String>,
    ) {
        self.config.agent_role_models.series = series;
    }
    ...
    fn collaboration_mode_indicator(&self) -> Option<CollaborationModeIndicator> {
        ...
        match self.active_mode_kind() {
            ...
            ModeKind::Heavy => Some(CollaborationModeIndicator::Heavy),
            ModeKind::HeavyPlan => Some(CollaborationModeIndicator::HeavyPlan),
    ...
    /// Cycle to the next collaboration mode preset in configured list order.
    ...
    fn on_prepared_context_response(&mut self, ev: PreparedContextResponseEvent) {
        if let Some(snapshot_json) = ev.snapshot_json {
            let mut lines = vec!["/context".magenta().into(), "".into()];
            lines.extend(snapshot_json.lines().map(|line| line.to_string().into()));
            self.add_plain_history_lines(lines);
            return;
        }

        self.add_info_message(
            "Prepared request context is not available yet.".to_string(),
            Some("Run at least one model turn, then try /context again.".to_string()),
        );
    }
