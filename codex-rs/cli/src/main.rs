...
fn should_force_official_codex_profile() -> bool {
    std::env::var_os("CODEX_LAUNCHED_FROM_CODER").is_none()
        && std::env::var_os("CODEX_ALLOW_CUSTOM_PLUGINS").is_none()
}
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    // Keep the `codex` command aligned with official defaults unless explicitly
    // launched from `coder` (or manually overridden by env var).
    if should_force_official_codex_profile() {
        root_config_overrides
            .raw_overrides
            .push("plugins.enabled=[]".to_string());
    }
