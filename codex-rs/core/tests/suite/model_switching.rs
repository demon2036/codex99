...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn plugin_agent_role_skips_model_switch_developer_message() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let role_prompt = "<identity>ruler role prompt</identity>";
    let role_prompt_for_hook = role_prompt.to_string();

    let mut builder = test_codex()
        .with_model("gpt-5.2-codex")
        .with_pre_build_hook(move |home| {
            let plugin_dir = home.join("plugins/testruler");
            let roles_dir = plugin_dir.join("roles/ruler");
            fs::create_dir_all(&roles_dir).expect("create test plugin roles dir");
...
[plugin]
id = "testruler"
default_agent_role = "ruler-heavy"

[roles.ruler-heavy]
kind = "spawnable"
label = "ruler (heavy)"
description = "test role"
prompt_file = "roles/ruler/heavy.md"
model = "gpt-5.2"
reasoning_effort = "xhigh"
read_only = true
allows_collab_tools = false
"#,
            )
            .expect("write test plugin manifest");
            fs::write(
                roles_dir.join("heavy.md"),
                format!("{role_prompt_for_hook}\n\n## Test role instructions"),
            )
            .expect("write test role prompt");
        })
        .with_config(|config| {
            let enabled = vec!["testruler".to_string()];
            config.plugins =
                PluginRegistry::load(&enabled, config.cwd.as_path(), config.codex_home.as_path())
                    .expect("load test plugin registry");
            config.agent_role = Some("ruler-heavy".to_string());
        });
    let test = builder.build(&server).await?;
    ...
    assert!(
        !developer_texts
            .iter()
            .any(|text| text.contains("<model_switch>")),
        "did not expect model switch message when plugin agent role is active"
    );
    assert!(
        developer_texts
            .iter()
            .any(|text| text.contains("ruler role prompt")),
        "expected plugin role prompt to remain present"
    );
