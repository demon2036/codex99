...
impl SessionConfiguration {
    ...
    pub(crate) fn apply(&self, updates: &SessionSettingsUpdate) -> ConstraintResult<Self> {
        ...
        if let Some(mut collaboration_mode) = updates.collaboration_mode.clone() {
            if let Some(override_instructions) = next_configuration
                .original_config_do_not_use
                .plugins
                .collaboration_mode_override(collaboration_mode.mode)
            {
...
impl Session {
    ...
    fn make_turn_context(
        ...
        let tools_config = ToolsConfig::new(&ToolsConfigParams {
            ...
            plugins: Some(&per_turn_config.plugins),
        });
    ...
    fn build_model_instructions_update_item(
        ...
        if next.config.agent_role.is_some() {
            return None;
        }
...
async fn submission_loop(sess: Arc<Session>, config: Arc<Config>, rx_sub: Receiver<Submission>) {
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        match sub.op.clone() {
            ...
            Op::GetPreparedContextRequest => {
                handlers::get_prepared_context_request(&sess, sub.id.clone()).await;
            }
            Op::ListMcpTools => {
...
mod handlers {
    ...
    pub async fn get_prepared_context_request(sess: &Arc<Session>, sub_id: String) {
        let snapshot_json = sess.services.model_client.prepared_context_snapshot().await;
        let event = Event {
            id: sub_id,
            msg: EventMsg::PreparedContextResponse(crate::protocol::PreparedContextResponseEvent {
                snapshot_json,
            }),
        };
        sess.send_event_raw(event).await;
...
async fn try_run_sampling_request(
    ...
    let plan_mode = turn_context.collaboration_mode.mode.is_plan_style();
...
mod tests {
    ...
    #[tokio::test]
    async fn collaboration_mode_updates_apply_plugin_overrides() {
        let codex_home = tempfile::tempdir().expect("create temp dir");

        let plugin_dir = codex_home
            .path()
            .join("plugins")
            .join("collab-prompts-test");
        std::fs::create_dir_all(plugin_dir.join("modes")).expect("create plugin dirs");

        std::fs::write(
            plugin_dir.join("plugin.toml"),
            r#"
[plugin]
id = "collab-prompts-test"
...
"#,
        ...
        std::fs::write(plugin_dir.join("modes/heavy_plan.md"), "OVERRIDE").expect("write prompt");

        std::fs::write(
            codex_home.path().join("config.toml"),
            r#"
[plugins]
enabled = ["collab-prompts-test"]
"#,
        )
        ...
        let collaboration_mode = CollaborationMode {
            mode: ModeKind::HeavyPlan,
            settings: Settings {
                model: model.clone(),
                reasoning_effort: None,
                developer_instructions: Some("BUILTIN".to_string()),
            },
        };
        let session_configuration = SessionConfiguration {
            provider: config.model_provider.clone(),
        ...
        assert_eq!(updated.collaboration_mode.mode, ModeKind::HeavyPlan);
        assert_eq!(
            updated
                .collaboration_mode
                .settings
    ...
    #[tokio::test]
    async fn get_prepared_context_request_returns_none_when_unset() {
        let (session, _turn_context, rx_event) = make_session_and_context_with_rx().await;

        handlers::get_prepared_context_request(&session, "prepared-ctx-1".to_string()).await;

        let event = rx_event
            .recv()
            .await
            .expect("expected prepared context event");
        assert_eq!(event.id, "prepared-ctx-1");
        match event.msg {
            EventMsg::PreparedContextResponse(ev) => {
                assert_eq!(ev.snapshot_json, None);
            }
            other => panic!("expected PreparedContextResponse, got {other:?}"),
        }
    }

    #[tokio::test]
    async fn get_prepared_context_request_returns_saved_snapshot() {
        let (session, _turn_context, rx_event) = make_session_and_context_with_rx().await;
        session
            .services
            .model_client
            .set_prepared_context_snapshot("{\"test\":true}".to_string())
            .await;

        handlers::get_prepared_context_request(&session, "prepared-ctx-2".to_string()).await;

        let event = rx_event
            .recv()
            .await
            .expect("expected prepared context event");
        assert_eq!(event.id, "prepared-ctx-2");
        match event.msg {
            EventMsg::PreparedContextResponse(ev) => {
                assert_eq!(ev.snapshot_json, Some("{\"test\":true}".to_string()));
            }
            other => panic!("expected PreparedContextResponse, got {other:?}"),
        }
    }
