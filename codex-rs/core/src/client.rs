...
struct ModelClientState {
    ...
    prepared_context_snapshot: RwLock<Option<String>>,
}
...
impl ModelClient {
    ...
    pub fn new(
        ...
        Self {
            state: Arc::new(ModelClientState {
                ...
                prepared_context_snapshot: RwLock::new(None),
            }),
        }
    }
    ...
    pub(crate) async fn set_prepared_context_snapshot(&self, snapshot_json: String) {
        let mut guard = self.state.prepared_context_snapshot.write().await;
        *guard = Some(snapshot_json);
    }

    pub async fn prepared_context_snapshot(&self) -> Option<String> {
        self.state.prepared_context_snapshot.read().await.clone()
    }
...
impl ModelClientSession {
    ...
    async fn stream_responses_api(
        ...
        loop {
            ...
            let snapshot_json = build_http_prepared_context_snapshot(&request, &options);
            self.client
                .set_prepared_context_snapshot(snapshot_json)
                .await;
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            let snapshot_json = build_websocket_prepared_context_snapshot(&ws_request, &options);
            self.client
                .set_prepared_context_snapshot(snapshot_json)
                .await;
...
fn build_http_prepared_context_snapshot(
    request: &ResponsesApiRequest,
    options: &ApiResponsesOptions,
) -> String {
    let turn_state = options
        .turn_state
        .as_ref()
        .and_then(|state| state.get().cloned());
    let payload = serde_json::json!({
        "transport": "responses_http",
        "request_type": "response_create",
        "model": &request.model,
        ...
        "text": &request.text,
        "conversation_id": options.conversation_id.as_ref(),
        "session_source": options.session_source.as_ref(),
        "extra_headers": headers_to_json_map(&options.extra_headers),
        "compression": compression_name(options.compression),
        "turn_state": turn_state,
    });
    serde_json::to_string_pretty(&payload)
        .unwrap_or_else(|error| format!("failed to serialize prepared context snapshot: {error}"))
}

fn build_websocket_prepared_context_snapshot(
    ...
    let payload = match request {
        ResponsesWsRequest::ResponseCreate(req) => serde_json::json!({
        ...
        ResponsesWsRequest::ResponseAppend(req) => serde_json::json!({
            "transport": "responses_websocket",
            "request_type": "response_append",
            "input": &req.input,
            "conversation_id": options.conversation_id.as_ref(),
            "session_source": options.session_source.as_ref(),
            "extra_headers": headers_to_json_map(&options.extra_headers),
...
fn headers_to_json_map(headers: &ApiHeaderMap) -> serde_json::Map<String, Value> {
    let mut map = serde_json::Map::new();
    for (key, value) in headers {
        let value_text = value
            .to_str()
            .map_or_else(|_| "<non-utf8>".to_string(), str::to_string);
        map.insert(key.to_string(), Value::String(value_text));
    }
    map
}
