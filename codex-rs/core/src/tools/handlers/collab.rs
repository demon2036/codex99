...
mod spawn {
    ...
    pub async fn handle(
        ...
        if turn.config.plugins.is_empty() {
            let agent_role = parse_builtin_agent_role(args.agent_type.as_deref())?;
            return spawn_builtin(session, turn, call_id, agent_role, input_items, prompt).await;
        }
        ...
        let caller_role = turn
            .config
            .agent_role
            .as_deref()
            .map(normalize_role_id)
            .unwrap_or_else(|| "default".to_string());
        let requested_role = args
            .agent_role
            .as_deref()
            .or(args.subagent_type.as_deref())
            .or(args.agent_type.as_deref())
            .map(normalize_role_id)
            .filter(|id| !id.is_empty());
        let target_role = resolve_target_role_id(
            &turn.config.plugins,
        ...
        let target_def = turn.config.plugins.role(&target_role).ok_or_else(|| {
            FunctionCallError::RespondToModel(format!("unknown agent role `{target_role}`"))
        })?;
        if target_def.kind != RoleKind::Spawnable {
            return Err(FunctionCallError::RespondToModel(format!(
                "agent role `{}` is not spawnable",
                target_def.id
            )));
        }

        let permission =
            spawn_permission(turn.config.as_ref(), &turn.config.plugins, &caller_role)?;
        if !permission.allows(&target_def.id) {
            return Err(FunctionCallError::RespondToModel(format!(
                "Agent role `{caller_role}` is not allowed to spawn `{}`. Allowed: {}.",
                target_def.id,
                permission.describe()
            )));
        }
        ...
        session
            .send_event(
                ...
                CollabAgentSpawnBeginEvent {
                    ...
                    agent_role: Some(target_def.id.clone()),
                    prompt: prompt.clone(),
                }
                .into(),
            )
        ...
        config.agent_role = Some(target_def.id.clone());
        if let Some(category) = category.as_deref()
            && turn.config.plugins.category(category).is_some()
        {
            let append = turn
                .config
                .plugins
                .read_category_prompt_append(category)
                .map_err(|err| {
                    FunctionCallError::RespondToModel(format!(
                        "failed to load category `{category}`: {err}"
                    ))
                })?;
            config.developer_instructions = Some(match config.developer_instructions.as_deref() {
                None => append,
                Some(existing) if existing.trim().is_empty() => append,
    ...
    async fn spawn_builtin(
        ...
        let requested_agent_role = builtin_agent_role_id(agent_role).to_string();
        let session_source = turn.session_source.clone();
        let child_depth = next_thread_spawn_depth(&session_source);
        if exceeds_thread_spawn_depth_limit(child_depth) {
            return Err(FunctionCallError::RespondToModel(
        ...
        session
            .send_event(
                ...
                CollabAgentSpawnBeginEvent {
                    call_id: call_id.clone(),
                    sender_thread_id: session.conversation_id,
                    agent_role: Some(requested_agent_role.clone()),
                    prompt: prompt.clone(),
                }
                .into(),
            )
        ...
        session
            .send_event(
                ...
                CollabAgentSpawnEndEvent {
                    ...
                    sender_thread_id: session.conversation_id,
                    new_thread_id,
                    agent_role: Some(requested_agent_role),
                    prompt,
                    status,
                }
                .into(),
    ...
    fn parse_role_variant(role: &str) -> Option<RoleVariant> {
        match role.split('-').next_back()? {
            "fast" => Some(RoleVariant::Fast),
            "normal" => Some(RoleVariant::Normal),
            "heavy" => Some(RoleVariant::Heavy),
            _ => None,
        }
    }

    fn builtin_agent_role_id(agent_role: AgentRole) -> &'static str {
        match agent_role {
    ...
    pub(super) fn resolve_target_role_id(
        plugins: &crate::plugins::PluginRegistry,
        requested_role: Option<&str>,
        category: Option<&str>,
        caller_role: &str,
    ) -> Result<String, FunctionCallError> {
        let caller_variant = parse_role_variant(caller_role).unwrap_or(RoleVariant::Normal);
        let Some(requested_role) = requested_role else {
            if category.is_none() {
                return Err(FunctionCallError::RespondToModel(
                    "agent_role (or subagent_type/agent_type) is required when plugins are enabled"
                        .to_string(),
                ));
            }

            let variant_default = format!("sisyphus-junior-{}", caller_variant.as_str());
            if plugins.role(&variant_default).is_some() {
                return Ok(variant_default);
            }
            if plugins.role("sisyphus-junior").is_some() {
                return Ok("sisyphus-junior".to_string());
            }
            return Err(FunctionCallError::RespondToModel(
                "missing default category agent role (expected `sisyphus-junior` or `sisyphus-junior-{variant}`)"
                    .to_string(),
            ));
        };

        if plugins.role(requested_role).is_some() {
            return Ok(requested_role.to_string());
        }

        if parse_role_variant(requested_role).is_some() {
            return Err(FunctionCallError::RespondToModel(format!(
                "unknown agent role `{requested_role}`"
            )));
        }

        let variant_target = format!("{requested_role}-{}", caller_variant.as_str());
        if plugins.role(&variant_target).is_some() {
            return Ok(variant_target);
        }
    ...
    fn parse_builtin_agent_role(raw: Option<&str>) -> Result<AgentRole, FunctionCallError> {
        match raw.map(str::trim).filter(|s| !s.is_empty()) {
            None => Ok(AgentRole::Default),
            Some("default") => Ok(AgentRole::Default),
            Some("worker") => Ok(AgentRole::Worker),
            Some("explorer") => Ok(AgentRole::Explorer),
            Some("orchestrator") => Ok(AgentRole::Orchestrator),
            Some(other) => Err(FunctionCallError::RespondToModel(format!(
                "invalid agent_type `{other}`"
            ))),
        }
    }
    ...
    impl SpawnPermission {
        ...
        fn describe(&self) -> String {
            match self {
                ...
                Self::AllowList(allowed) if allowed.is_empty() => "(none)".to_string(),
                Self::AllowList(allowed) => allowed.iter().cloned().collect::<Vec<_>>().join(", "),
            }
        }
    }
    ...
    fn spawn_permission(
        ...
        plugins: &crate::plugins::PluginRegistry,
        caller_role: &str,
    ) -> Result<SpawnPermission, FunctionCallError> {
        if let Some(permission) = config_spawn_permission(config, plugins, caller_role)? {
            return Ok(permission);
        }
        if let Some(permission) = plugin_default_spawn_permission(plugins, caller_role) {
            return Ok(permission);
        }
        Ok(SpawnPermission::None)
    }

    fn plugin_default_spawn_permission(
        plugins: &crate::plugins::PluginRegistry,
        caller_role: &str,
    ) -> Option<SpawnPermission> {
        let allowed_raw = plugins.spawn_defaults_allow().get(caller_role)?;
        if allowed_raw
            .iter()
            .any(|raw| raw.trim() == "*" || raw.trim().eq_ignore_ascii_case("all"))
        {
            return Some(SpawnPermission::All);
        }
        ...
        Some(if allowed.is_empty() {
            SpawnPermission::None
        } else {
            SpawnPermission::AllowList(allowed)
        })
    }

    fn config_spawn_permission(
        config: &Config,
        plugins: &crate::plugins::PluginRegistry,
        caller_role: &str,
    ) -> Result<Option<SpawnPermission>, FunctionCallError> {
        for (caller, allowed_raw) in &config.agent_spawn.allow {
            if normalize_role_id(caller) != caller_role {
                continue;
            }
            if allowed_raw
                .iter()
                .any(|raw| raw.trim() == "*" || raw.trim().eq_ignore_ascii_case("all"))
            {
                return Ok(Some(SpawnPermission::All));
            }

            let mut allowed = HashSet::new();
            for raw in allowed_raw {
                let target = normalize_role_id(raw);
                let Some(role) = plugins.role(&target) else {
                    return Err(FunctionCallError::RespondToModel(format!(
                        "Invalid agent_spawn.allow entry for {caller_role}: unknown role `{raw}`",
                    )));
                };
                if role.kind == RoleKind::Spawnable {
                    allowed.insert(role.id.clone());
                }
            }

            return Ok(Some(if allowed.is_empty() {
                SpawnPermission::None
            } else {
                SpawnPermission::AllowList(allowed)
            }));
...
mod tests {
    ...
    fn load_test_plugins() -> PluginRegistry {
        let codex_home = tempfile::tempdir().expect("create temp codex home");
        let plugin_dir = codex_home.path().join("plugins").join("ohlike");
        fs::create_dir_all(plugin_dir.join("roles/explore")).expect("create explore roles");
        fs::create_dir_all(plugin_dir.join("roles/sisyphus-junior"))
            .expect("create sisyphus-junior roles");
...
[roles.explore-fast]
kind = "spawnable"
prompt_file = "roles/explore/fast.md"

[roles.explore-normal]
kind = "spawnable"
prompt_file = "roles/explore/normal.md"

[roles.sisyphus-junior-fast]
kind = "spawnable"
prompt_file = "roles/sisyphus-junior/fast.md"
"#,
        )
        .expect("write plugin.toml");

        fs::write(plugin_dir.join("roles/explore/fast.md"), "").expect("write fast prompt");
        fs::write(plugin_dir.join("roles/explore/normal.md"), "").expect("write normal prompt");
        fs::write(plugin_dir.join("roles/sisyphus-junior/fast.md"), "")
            .expect("write sj fast prompt");
    ...
    #[test]
    fn spawn_target_resolves_unqualified_role_using_caller_variant() {
        let plugins = load_test_plugins();
        let resolved =
            spawn::resolve_target_role_id(&plugins, Some("explore"), None, "atlas-fast").unwrap();
        assert_eq!(resolved, "explore-fast");
    }

    #[test]
    fn spawn_target_resolves_category_default_using_caller_variant() {
        let plugins = load_test_plugins();
        let resolved =
            spawn::resolve_target_role_id(&plugins, None, Some("quick"), "atlas-fast").unwrap();
        assert_eq!(resolved, "sisyphus-junior-fast");
    }
