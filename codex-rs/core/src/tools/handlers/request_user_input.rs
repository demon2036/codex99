...
mod tests {
    ...
    #[test]
    fn request_user_input_mode_availability_is_plan_style_only() {
        assert!(ModeKind::Plan.allows_request_user_input());
        assert!(ModeKind::HeavyPlan.allows_request_user_input());
        assert!(!ModeKind::Default.allows_request_user_input());
        assert!(!ModeKind::Heavy.allows_request_user_input());
    ...
    fn request_user_input_unavailable_messages_use_default_name_for_default_modes() {
        ...
        assert_eq!(
            request_user_input_unavailable_message(ModeKind::HeavyPlan),
            None
        );
        assert_eq!(
        ...
        assert_eq!(
            request_user_input_unavailable_message(ModeKind::Heavy),
            Some("request_user_input is unavailable in Heavy mode".to_string())
        );
        assert_eq!(
    ...
    #[test]
    fn request_user_input_tool_description_mentions_plan_style_modes() {
        assert_eq!(
            request_user_input_tool_description(),
            "Request user input for one to three short questions and wait for the response. This tool is only available in Plan or Heavy Plan mode.".to_string()
        );
    }
}
