...
pub(crate) struct ToolsConfig {
    ...
    pub plugin_spawnable_roles: Vec<String>,
...
pub(crate) struct ToolsConfigParams<'a> {
    ...
    pub(crate) plugins: Option<&'a crate::plugins::PluginRegistry>,
}
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        Self {
            ...
            plugin_spawnable_roles: plugins
                .map(|registry| {
                    registry
                        .spawnable_roles()
            ...
            plugin_categories: plugins
                .map(|registry| registry.categories().map(|cat| cat.id.clone()).collect())
                .unwrap_or_default(),
        }
    }
}
...
fn create_spawn_agent_tool(config: &ToolsConfig) -> ToolSpec {
    ...
    let role_hint = if config.plugin_spawnable_roles.is_empty() {
        format!(
        ...
        format!(
            "Optional agent role ({}). Use an explicit role when delegating.",
            config.plugin_spawnable_roles.join(", ")
        )
    };
    properties.insert(
        "agent_type".to_string(),
        JsonSchema::String {
            description: Some(role_hint),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Optional plugin-defined agent role id. Preferred over agent_type when plugins are enabled."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some("Alias for agent_role (Oh-My-style prompts).".to_string()),
        },
    );
    properties.insert(
        "category".to_string(),
        JsonSchema::String {
            description: Some(if config.plugin_categories.is_empty() {
                "Optional plugin-defined category id.".to_string()
            } else {
                format!(
                    "Optional plugin-defined category id ({}).",
                    config.plugin_categories.join(", ")
                )
            }),
        },
    );
