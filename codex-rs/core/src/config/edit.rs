...
mod document_helpers {
    ...
    pub(super) fn array_from_iter<I>(iter: I) -> TomlItem
    where
...
impl ConfigEditsBuilder {
    ...
    /// Replace the entire `[agent_spawn.allow]` table (per-caller allowlist of spawn targets).
    ///
    /// When `allow` is empty, the key is removed from the config file.
    pub fn set_agent_spawn_allow(mut self, allow: &HashMap<String, Vec<String>>) -> Self {
        if allow.is_empty() {
            self.edits.push(ConfigEdit::ClearPath {
                segments: vec!["agent_spawn".to_string(), "allow".to_string()],
            });
            return self;
        }

        let mut callers: Vec<_> = allow.iter().collect();
        callers.sort_by(|(left, _), (right, _)| left.cmp(right));

        let mut allow_table = TomlTable::new();
        allow_table.set_implicit(false);
        for (caller, targets) in callers {
            let mut targets = targets.clone();
            targets.sort();
            allow_table.insert(
                caller,
                document_helpers::array_from_iter(targets.into_iter()),
            );
        }

        self.edits.push(ConfigEdit::SetPath {
            segments: vec!["agent_spawn".to_string(), "allow".to_string()],
            value: TomlItem::Table(allow_table),
        });
        self
    }

    /// Replace the entire `[agent_role_models.series]` table.
    ///
    /// When `series` is empty, the key is removed from the config file.
    pub fn set_agent_role_models_series(
        mut self,
        series: &HashMap<AgentRoleModelSeries, String>,
    ) -> Self {
        if series.is_empty() {
            self.edits.push(ConfigEdit::ClearPath {
                segments: vec!["agent_role_models".to_string(), "series".to_string()],
            });
            return self;
        }
        ...
        let mut series_table = TomlTable::new();
        series_table.set_implicit(false);
        for (role_series, model) in entries {
            series_table.insert(role_series.as_str(), value(model.clone()));
        }

        self.edits.push(ConfigEdit::SetPath {
            segments: vec!["agent_role_models".to_string(), "series".to_string()],
            value: TomlItem::Table(series_table),
        });
        self
    }
...
"#;
    ...
    #[tokio::test]
    async fn async_builder_set_agent_role_models_series_persists() {
        let tmp = tempdir().expect("tmpdir");
        let codex_home = tmp.path().to_path_buf();
        ...
        let contents =
            std::fs::read_to_string(codex_home.join(CONFIG_TOML_FILE)).expect("read config");
        let expected = r#"[agent_role_models.series]
heavy = "gpt-5.3-codex"
normal = "gpt-5.2"
"#;
        assert_eq!(contents, expected);
        ...
        ConfigEditsBuilder::new(&codex_home)
            .set_agent_role_models_series(&HashMap::new())
            .apply()
            .await
            .expect("clear");

        let cleared =
            std::fs::read_to_string(codex_home.join(CONFIG_TOML_FILE)).expect("read cleared");
        assert_eq!(cleared, "");
    }
