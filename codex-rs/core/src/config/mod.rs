...
/// Settings that control which plugins are enabled.
#[derive(Serialize, Deserialize, Debug, Clone, Default, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct PluginsToml {
    /// Enabled plugin ids. Plugins are discovered under `plugins/<id>/plugin.toml`.
    #[serde(default)]
    pub enabled: Vec<String>,
}

/// Configuration for which agent roles are allowed to spawn other agent roles.
///
/// This is intended to support Oh-My-OpenCode-style systems where some executor
/// agents are allowed to spawn *read-only* research agents without granting
/// full orchestration.
#[derive(Serialize, Deserialize, Debug, Clone, Default, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct AgentSpawnConfig {
    /// A per-caller allowlist of spawn targets, keyed by agent role id.
    ///
    /// Example:
    /// ```toml
    /// [agent_spawn]
    /// allow = { "atlas" = ["*"], "sisyphus-junior" = ["explore", "librarian"] }
    /// ```
    #[serde(default)]
    pub allow: HashMap<String, Vec<String>>,
}

/// Named role-series suffixes used for batch model overrides.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Hash, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AgentRoleModelSeries {
    Fast,
    Normal,
    Heavy,
}

impl AgentRoleModelSeries {
    ...
    pub fn from_role_id(role_id: &str) -> Option<Self> {
        let (_, suffix) = role_id.rsplit_once('-')?;
        Self::parse(suffix)
    }
}

/// Persistent role-model overrides for role-id series suffixes.
#[derive(Serialize, Deserialize, Debug, Clone, Default, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct AgentRoleModelsConfig {
    /// Per-series model overrides (for example: `heavy = "gpt-5.3-codex"`).
    #[serde(default)]
    pub series: HashMap<AgentRoleModelSeries, String>,
}

impl AgentRoleModelsConfig {
    fn normalized(self) -> Self {
        let series = self
            .series
            .into_iter()
            .filter_map(|(series, model)| {
                let trimmed = model.trim();
                if trimmed.is_empty() {
                    None
                } else {
                    Some((series, trimmed.to_string()))
                }
            })
            .collect();
        Self { series }
    ...
    pub fn model_for_role(&self, role_id: &str) -> Option<&str> {
        let series = AgentRoleModelSeries::from_role_id(role_id)?;
        self.series.get(&series).map(String::as_str)
    }
}
...
pub struct Config {
    ...
    /// Optional agent role/persona identifier for the session (for example: "atlas").
    pub agent_role: Option<String>,

    /// Agent-to-agent spawn policy overrides.
    pub agent_spawn: AgentSpawnConfig,

    /// Per-series role model overrides (`fast` / `normal` / `heavy`).
    pub agent_role_models: AgentRoleModelsConfig,

    /// Loaded plugin definitions for this session.
    ...
    /// Start the TUI in the specified collaboration mode (default/plan/heavy/heavy_plan).
...
pub struct ConfigToml {
    ...
    /// Enabled plugins that can extend agent behavior (roles, prompts, etc.).
    ...
    /// Agent-to-agent spawn policy overrides.
    pub agent_spawn: Option<AgentSpawnConfig>,

    /// Optional per-series role model overrides used for plugin agent roles.
    pub agent_role_models: Option<AgentRoleModelsConfig>,
...
impl Config {
    ...
    fn load_config_with_layer_stack(
        ...
        let enabled_plugins = cfg
            ...
            .collect::<Vec<_>>();
        let plugins =
            crate::plugins::PluginRegistry::load(&enabled_plugins, &resolved_cwd, &codex_home)
                .map_err(|err| {
                    std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
        ...
        let mut config = Self {
            ...
            agent_spawn: cfg.agent_spawn.unwrap_or_default(),
            agent_role_models: cfg.agent_role_models.unwrap_or_default().normalized(),
            plugins,
            compact_prompt,
        ...
        if !config.plugins.is_empty() {
            if config.agent_role.is_none() {
                config.agent_role = config.plugins.default_agent_role().map(str::to_string);
            }
            // Plugins like OhLike rely on sub-agent tooling by default.
...
pub(crate) fn apply_plugin_agent_role(
    config: &mut Config,
    requested_role: &str,
) -> std::io::Result<()> {
    let Some(role) = config.plugins.role(requested_role) else {
        ...
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            format!("unknown agent_role `{requested_role}` (available: {available})"),
        ));
    };

    config.agent_role = Some(role.id.clone());

    let role_prompt = config.plugins.read_role_prompt(&role.id).map_err(|err| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("failed to load role prompt `{}`: {err}", role.id),
        )
    })?;

    config.developer_instructions = Some(match config.developer_instructions.take() {
        None => role_prompt,
        Some(existing) if existing.trim().is_empty() => role_prompt,
        Some(existing) => format!("{role_prompt}\n\n{existing}"),
    });

    if let Some(model_override) = config.agent_role_models.model_for_role(&role.id) {
        config.model = Some(model_override.to_string());
    } else if let Some(model) = role.model.clone() {
    ...
    if role.read_only {
        config
            ...
            .map_err(|err| {
                std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    format!("sandbox_policy is invalid: {err}"),
                )
            })?;
    }
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                agent_role_models: AgentRoleModelsConfig::default(),
...
mod notifications_tests {
    use crate::config::AgentRoleModelSeries;
    use crate::config::AgentRoleModelsConfig;
    use crate::config::Config;
    use crate::config::ConfigOverrides;
    use crate::config::ConfigToml;
    use crate::config::PluginsToml;
    use crate::config::apply_plugin_agent_role;
    use crate::config::types::NotificationMethod;
    use crate::config::types::Notifications;
    ...
    #[test]
    fn parse_agent_role_models_series() {
...
"#,
        ...
        let role_models = parsed
            .agent_role_models
            .expect("agent_role_models should exist");
        assert_eq!(
            role_models.series.get(&AgentRoleModelSeries::Heavy),
            Some(&"gpt-5.3-codex".to_string())
        );
        assert_eq!(
            role_models.series.get(&AgentRoleModelSeries::Normal),
            Some(&"gpt-5.2".to_string())
        );
    }

    #[test]
    fn apply_plugin_agent_role_prefers_series_model_override() {
        let codex_home = tempfile::tempdir().expect("create temp codex home");
        let plugin_dir = codex_home.path().join("plugins").join("ohlike");
        std::fs::create_dir_all(plugin_dir.join("roles")).expect("create plugin dirs");
        std::fs::write(
...
[roles.atlas-heavy]
kind = "spawnable"
prompt_file = "roles/atlas-heavy.md"
model = "gpt-5.2"
"#,
        )
        .expect("write plugin.toml");
        std::fs::write(
            plugin_dir.join("roles/atlas-heavy.md"),
            "atlas heavy prompt",
        )
        ...
        let mut config = Config::load_from_base_config_with_overrides(
            ConfigToml {
                plugins: Some(PluginsToml {
                    enabled: vec!["ohlike".to_string()],
                }),
                ..Default::default()
            },
            ConfigOverrides {
        ...
        config.agent_role_models = AgentRoleModelsConfig {
            series: HashMap::from([(AgentRoleModelSeries::Heavy, "gpt-5.3-codex".to_string())]),
        };

        apply_plugin_agent_role(&mut config, "atlas-heavy").expect("apply plugin role");
