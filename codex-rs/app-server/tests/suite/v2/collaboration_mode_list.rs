...
//! includes the default, plan, heavy, and heavy plan modes with their default model and
//! reasoning effort
...
/// Builds the heavy preset that the list response is expected to return.
fn heavy_preset() -> CollaborationModeMask {
    let presets = builtin_collaboration_mode_presets();
    presets
        .into_iter()
        .find(|p| p.mode == Some(ModeKind::Heavy))
        .unwrap()
}

/// Builds the heavy plan preset that the list response is expected to return.
fn heavy_plan_preset() -> CollaborationModeMask {
    let presets = builtin_collaboration_mode_presets();
    presets
        .into_iter()
        .find(|p| p.mode == Some(ModeKind::HeavyPlan))
        .unwrap()
}
