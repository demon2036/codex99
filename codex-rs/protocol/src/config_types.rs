...
impl ModeKind {
    ...
    pub const fn is_tui_visible(self) -> bool {
        matches!(
            self,
            Self::Plan | Self::HeavyPlan | Self::Default | Self::Heavy
        )
    }

    pub const fn is_plan_style(self) -> bool {
        matches!(self, Self::Plan | Self::HeavyPlan)
    }
...
mod tests {
    ...
    #[test]
    fn mode_kind_plan_style_and_user_input_availability_flags_match() {
        let expected = [
            (ModeKind::Plan, true, true),
            (ModeKind::HeavyPlan, true, true),
            (ModeKind::Default, false, false),
            (ModeKind::Heavy, false, false),
            (ModeKind::PairProgramming, false, false),
            (ModeKind::Execute, false, false),
        ];

        for (mode, is_plan_style, allows_request_user_input) in expected {
            assert_eq!(mode.is_plan_style(), is_plan_style);
            assert_eq!(mode.allows_request_user_input(), allows_request_user_input);
        }
    }
}
