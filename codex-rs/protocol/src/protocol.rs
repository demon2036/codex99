...
pub enum Op {
    ...
    /// Request the most recently prepared Responses API request snapshot.
    /// Reply is delivered via `EventMsg::PreparedContextResponse`.
    GetPreparedContextRequest,
...
pub enum EventMsg {
    ...
    /// Most recently prepared Responses API request snapshot.
    PreparedContextResponse(PreparedContextResponseEvent),
...
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema, TS)]
pub struct PreparedContextResponseEvent {
...
mod tests {
    ...
    #[test]
    fn get_prepared_context_request_serialization() -> Result<()> {
        let op = Op::GetPreparedContextRequest;
        let json_op = serde_json::to_value(op)?;
        assert_eq!(json_op, json!({ "type": "get_prepared_context_request" }));
        Ok(())
    }

    #[test]
    fn prepared_context_response_event_roundtrip() -> Result<()> {
        let event = Event {
            id: "1".to_string(),
            msg: EventMsg::PreparedContextResponse(PreparedContextResponseEvent {
                snapshot_json: Some("{\"hello\":\"world\"}".to_string()),
            }),
        };

        let serialized = serde_json::to_value(&event)?;
        let deserialized: Event = serde_json::from_value(serialized)?;
        assert_eq!(deserialized.id, "1");
        match deserialized.msg {
            EventMsg::PreparedContextResponse(PreparedContextResponseEvent { snapshot_json }) => {
                assert_eq!(snapshot_json, Some("{\"hello\":\"world\"}".to_string()));
            }
            other => panic!("expected PreparedContextResponse event, got {other:?}"),
        }
        Ok(())
    }
